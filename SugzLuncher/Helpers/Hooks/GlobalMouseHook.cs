...
namespace SugzLuncher.Helpers
{
    ...
    public class GlobalMouseHook
    {
        ...
        bool MouseWheelEvent(GlobalMouseHookEventArgs e)
        {
            ...
            WheelRotation wheelRotation = (WheelRotation)(GlobalHookHelper.GetSignedHWORD(e.MouseData.mouseData)); //The Wheel Delta will always be either 120 for forwards movement and -120 for backwards movement which is already declared on the WheelRotation enum. All we have to do is cast the Wheel Delta to WheelRotation.
        ...
        GHMouseButtons MouseMessageToGHMouseButton(InternalGlobalMouseHook.MouseMessages button, uint mouseData = 0)
        {
            switch (button) //We need to check which button was pressed and we need to return the associated GHMouseButton enumeration.
            {
                ...
                case InternalGlobalMouseHook.MouseMessages.WM_XBUTTONDOWN:
                    if ((InternalGlobalMouseHook.XButtons)GlobalHookHelper.GetUnsignedHWORD(mouseData) == InternalGlobalMouseHook.XButtons.WM_XBUTTON1) //The high order word stored in mouseData tells us which button was clicked. We need to get that by using the GetUnsignedHWord helper method.
                        return GHMouseButtons.XButton1;
                    else
                        return GHMouseButtons.XButton2;
                case InternalGlobalMouseHook.MouseMessages.WM_XBUTTONUP:
                    if ((InternalGlobalMouseHook.XButtons)GlobalHookHelper.GetUnsignedHWORD(mouseData) == InternalGlobalMouseHook.XButtons.WM_XBUTTON1)
                        return GHMouseButtons.XButton1;
                    else
                        return GHMouseButtons.XButton2;
                default:
